...
class ResUsers(models.Model):
    ...
    #增加给浏览器发送消息
    def notify_sendone(self, message="Default message", title=None, user_partner_id=False):
        ...
        title = title or _("Default")
        if user_partner_id == False:
            user_partner_id = self.partner_id.id

        message = "9999"

        self.env['bus.bus'].sendone((self._cr.dbname, 'res.partner', user_partner_id),
